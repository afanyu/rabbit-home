...
import BreedRequest from "../models/BreedRequest.js";
...
const createProductReview = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (product) {
      const breeder = {
        breeder_id: product?._id,
        owner: product?.owner,
        breeder: product?.name,
      };
      // console.log(product)
      const breedRequest = new BreedRequest({ ...req.body, breeder });
      // console.log(breedRequest)
      await breedRequest.save();
      res.status(201).json(breedRequest);
    } else {
      res.status(404);
      throw new Error("Product not found");
    }
