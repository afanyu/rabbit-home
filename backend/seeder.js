...
const importData = async () => {
  try {
    ...
    const sampleProducts = products.map((product) => {
      ...
      const owner = getRandomOwner(owners);
      return { ...product, user: adminUser, owner };
    });
    // Migrate products
...
function getRandomOwner(owners) {
  const randomIndex = Math.floor(Math.random() * owners.length - 1);
